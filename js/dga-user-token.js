(function($) {
    ...
    // ฟังก์ชั่นสำหรับการตั้งค่า event listeners
    function setupEventListeners() {
        ...
        $(window).on('click', function(event) {
            if ($(event.target).is('#dga-token-modal') || $(event.target).is('#dga-delete-modal')) {
                closeAllModals();
            }
        });
        ...
        $('#dga-api-key').on('keypress', function(e) {
            if (e.which === 13) {
                saveApiKey();
                return false;
            }
    ...
    // เริ่มการทำงานเมื่อ document พร้อม
